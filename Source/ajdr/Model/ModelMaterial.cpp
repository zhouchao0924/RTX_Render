...
void FSurfaceParameterVector2::Serialize(FArchive &Ar, uint32 Ver)
{
	if (Ar.IsSaving() || Ar.IsLoading())
	{
		Ar << Value;
...
void FSurfaceParameterVector3::Serialize(FArchive &Ar, uint32 Ver)
{
	if (Ar.IsSaving() || Ar.IsLoading())
	{
		Ar << Value;
...
//////////////////////////////////////////////////////////////////////////
void FModelMaterial::Serialize(bool bEmbedModel, FArchive &Ar, uint32 Ver)
{
	if (Ar.IsSaving())
	{
		...
		NumParas = OverrideVec2Parameters.Num();
		Ar << NumParas;
		for (int i = 0; i < OverrideVec2Parameters.Num(); ++i)
		{
			OverrideVec2Parameters[i].Serialize(Ar, Ver);
		}
		
		NumParas = OverrideVec3Parameters.Num();
		Ar << NumParas;
		for (int i = 0; i < OverrideVec3Parameters.Num(); ++i)
		{
	...
	else if (Ar.IsLoading())
	{
		...
		if ( (bEmbedModel && Ver > MODELFILE_BODY_VER_7) || (!bEmbedModel && Ver> SURFACE_BODY_VER_1))
		{
			Ar << NumParas;
			OverrideVec2Parameters.SetNum(NumParas);
			for (int i = 0; i < OverrideVec2Parameters.Num(); ++i)
			{
				OverrideVec2Parameters[i].Serialize(Ar, Ver);
			}
		...
		if ((bEmbedModel && Ver > MODELFILE_BODY_VER_8) || (!bEmbedModel && Ver > SURFACE_BODY_VER_2))
		{
			Ar << Name;
		}
	}
...
void FModelMaterialUE4::Serialize(bool bEmbedModel, FArchive &Ar, uint32 Ver)
{
	FModelMaterial::Serialize(bEmbedModel, Ar, Ver);
...
void FModelMaterialRef::Serialize(bool bEmbedModel, FArchive &Ar, uint32 Ver)
{
	FModelMaterial::Serialize(bEmbedModel, Ar, Ver);
...
UMaterialInterface *FModelMaterialRef::GetUE4Material()
{
	...
	UModelFile *Model = Cast<UModelFile>(RefSurface.Get());
	if (Model)
	{
		return Model->GetUE4Material(0);
	}
