...
void USurfaceFile::FHeader::Serialize(FArchive &Ar)
{
	...
	else if (Ar.IsLoading())
	{
		...
		if (HeadVersion > SURFACE_HEADERVER_5)
		{
			Ar << DisplayName;
		}
	}
...
void USurfaceFile::SetDisplayName(const FString &Name)
{
	m_Header.DisplayName = Name;
	MarkDirty(true);
}

FString USurfaceFile::GetDisplayName()
{
	return m_Header.DisplayName;
}

void USurfaceFile::SetMaterialName(const FString &Name)
{
	FModelMaterial *material = GetMaterial();
	if (material)
	{
...
FString USurfaceFile::GetMaterialName()
{
	FModelMaterial* Material = GetMaterial();
...
FString USurfaceFile::GetSurfaceChineseName( )
{
	if (DRInfo == NULL)
	{
		return TEXT("");
	...
	if (OutSurfaceName.Len() <= 0)
	{
		...
		if (Summary)
		{
			OutSurfaceName = Summary->ResourceName;
			DRInfo->SetStringField("ChineseName", OutSurfaceName);
		}
	}
	return OutSurfaceName;
}

void USurfaceFile::SetSurfaceChineseName( const FString & InSurfaceName)
{
	if(DRInfo==NULL)
	{ 
		return;
	...
	if (Summary)
	{
		Summary->ResourceName= InSurfaceName;
...
UMaterialInstanceDynamic * USurfaceFile::ResetMaterial()
{
	FModelMaterial* Material = GetMaterial();
	...
	if (Material)
	{
		...
		return GetDynamicMaterialInstance();
	}

	return nullptr;
}

void USurfaceFile::UnloadResource()
{
	Unload();
}

EMaterialType USurfaceFile::GetSurfaceType()
{
	FModelMaterial* Material = GetMaterial();
	if (Material)
	{
...
void USurfaceFile::SerializeHeader(FArchive &Ar)
{
	...
	if (Ar.IsSaving())
	{
		...
		if (m_MaterialInfo)
		{
			m_MaterialInfo->Serialize(false, Ar, m_Header.BodyVersion);
		}
	}
	else if(Ar.IsLoading())
	{
