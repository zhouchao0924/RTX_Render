...
{
	const static float Position_Tolerance = 0.001f;
	const static float Normal_Tolerance = 0.001f;
	const static float UV_Tolerance = 0.001f;

	bool IsVertSame(const FProcMeshVertex &V0, const FProcMeshVertex &V1)
	{
		if (!V0.Normal.Equals(V1.Normal, Normal_Tolerance))
		{
			return false;
		}
		...
		if (!V0.Position.Equals(V1.Position, Position_Tolerance))
		{
			return false;
		}

		if (V0.Tangent.bFlipTangentY != V1.Tangent.bFlipTangentY)
		{
			return false;
		}

		if (!V0.Tangent.TangentX.Equals(V1.Tangent.TangentX, Normal_Tolerance))
		{
			return false;
		}

		if (!V0.UV0.Equals(V1.UV0, UV_Tolerance))
		{
			return false;
		}
	...
	inline int32 FindIndex(TArray<FProcMeshVertex> &Verts, const FProcMeshVertex &V)
	{
		int32 FoundIndex = INDEX_NONE;

		for (int32 i = 0; i < Verts.Num(); ++i)
		{
			FProcMeshVertex &Vert = Verts[i];
			if (IsVertSame(Vert, V))
			{
				FoundIndex = i;
				break;
			}
	...
	void CompressMeshSection(FProcMeshSection &Section)
	{
		TArray<FProcMeshVertex> UniqueVerts;

		TArray<uint32> RemIndices;
		RemIndices.SetNum(Section.ProcIndexBuffer.Num());

		for (int32 i = 0; i < Section.ProcVertexBuffer.Num(); ++i)
		{
			FProcMeshVertex &Vert = Section.ProcVertexBuffer[i];
			int32 FoundIndex = FindIndex(UniqueVerts, Vert);
			if (FoundIndex == INDEX_NONE)
			{
				FoundIndex = UniqueVerts.Add(Vert);
			}
			RemIndices[i] = FoundIndex;
		}

		Section.ProcIndexBuffer = RemIndices;
		Section.ProcVertexBuffer = UniqueVerts;
	}

	void CompressModel(FModel *InModel)
	{
		for (int32 i = 0; i < InModel->Sections.Num(); ++i)
		{
			FProcMeshSection &Section = InModel->Sections[i];
			CompressMeshSection(Section);
		}
	}
	...
	bool CompressTexture(FModelTexture *Tex)
	{
		if (Tex)
		{
			ETexSlot slot = Tex->Source.CompressionSettings == TC_Normalmap ? ETexSlot::eNormal : ETexSlot::eDiffuse;
			return CompressUtil::CompressTexture(&Tex->Source, slot);
		}
		return false;
	}
	...
	bool  CompressTexture(FTextureSourceInfo *TexInfo, ETexSlot slot)
	{
		...
		FImage* SourceMip = new(SourceMips) FImage(TexInfo->SizeX, TexInfo->SizeY, 1, ImageFormat,
			TexInfo->SRGB ? (TexInfo->bUseLegacyGamma ? EGammaSpace::Pow22 : EGammaSpace::sRGB) : EGammaSpace::Linear);

		FMemory::Memcpy(SourceMip->RawData.GetData(), TexInfo->Data.GetData(), TexInfo->Data.Num());
		...
		bool bNormalmap = slot == eNormal;

		// helps to reduce aliasing further
// 		DefaultSettings.MipSharpening = -4.0f;
		DefaultSettings.SharpenMipKernelSize = 2;
// 		DefaultSettings.bApplyKernelToTopMip = true;
		//DefaultSettings.bRenormalizeTopMip = bNormalmap;
		DefaultSettings.bSRGB = 1/*bNormalmap ? 1 : 0*/;
 		DefaultSettings.MipGenSettings = TextureMipGenSettings::TMGS_FromTextureGroup;
		DefaultSettings.CompositeTextureMode = ECompositeTextureMode::CTM_NormalRoughnessToAlpha;

		TexInfo->CompressionSettings = bNormalmap ? TC_Normalmap : TC_Default;
		DefaultSettings.TextureFormatName = GetTextureCompressFormat(TexInfo);

		return BuildTexture(SourceMips, DefaultSettings, TexInfo->CompressedImages);
	}
	...
	FName GetTextureCompressFormat(FTextureSourceInfo *TexInfo)
	{
		...
		bool bNoCompression =  (TexInfo->SizeX < 4) || (TexInfo->SizeY < 4) ||
							   (TexInfo->SizeX % 4 != 0)|| (TexInfo->SizeY % 4 != 0);
...
// 		}
		...
		ETextureSourceFormat SourceFormat = TexInfo->SourceFormat;

		bool bHDRSource = ((SourceFormat == TSF_BGRE8) || (SourceFormat == TSF_RGBA16F));
		...
		if (bNoCompression)
		{
			...
			else if (SourceFormat == TSF_G8 || TexInfo->CompressionSettings == TC_Grayscale)
			{
				TextureFormatName = NameG8;
			}
			else if (TexInfo->CompressionSettings == TC_Normalmap && bUseDXT5NormalMap)
			{
		...
		else if (TexInfo->CompressionSettings == TC_HDR)
		{
			TextureFormatName = NameRGBA16F;
		}
		else if (TexInfo->CompressionSettings == TC_Normalmap)
		{
			TextureFormatName = bUseDXT5NormalMap ? NameDXT5n : NameBC5;
		}
		else if (TexInfo->CompressionSettings == TC_Displacementmap)
		{
			TextureFormatName = NameG8;
		}
		else if (TexInfo->CompressionSettings == TC_VectorDisplacementmap)
		{
			TextureFormatName = NameBGRA8;
		}
		else if (TexInfo->CompressionSettings == TC_Grayscale)
		{
			TextureFormatName = NameG8;
		}
		else if (TexInfo->CompressionSettings == TC_Alpha)
		{
			TextureFormatName = NameBC4;
		}
		else if (TexInfo->CompressionSettings == TC_DistanceFieldFont)
		{
			TextureFormatName = NameG8;
		}
		else if (TexInfo->CompressionSettings == TC_HDR_Compressed)
		{
			TextureFormatName = NameBC6H;
		}
		else if (TexInfo->CompressionSettings == TC_BC7)
		{
			TextureFormatName = NameBC7;
		}
		else if (TexInfo->CompressionNoAlpha)
		{
			TextureFormatName = NameDXT1;
		}
		else if (TexInfo->bDitherMipMapAlpha)
		{
	...
	*/
	...
	{
		static class ITargetPlatformManagerModule* TPM = NULL;
		if (!TPM)
		{
			TPM = FModuleManager::LoadModulePtr<ITargetPlatformManagerModule>("TargetPlatform");
		}
