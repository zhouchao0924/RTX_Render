...
void UModelFile::Serialize(FArchive &Ar)
{
	...
	if (Ar.IsSaving())
	{
		...
		for (int32 iMaterial = 0; iMaterial < NumMaterials; ++iMaterial)
		{
			...
			m_Materials[iMaterial]->Serialize(true, Ar, m_Header.BodyVersion);
	...
	else if (Ar.IsLoading())
	{
		...
		for (int32 iModel = 0; iModel < m_Header.Models.Num(); ++iModel)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UModelFile new FModel cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
			Time1 = FDateTime::Now();
			newModel->Serialize(Ar, m_Header.BodyVersion);
			UE_LOG(LogTemp, Log, TEXT("UModelFile Serialize FModel cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
			Time1 = FDateTime::Now();
		...
		for (int32 iMaterial = 0; iMaterial < m_Header.Materials.Num(); ++iMaterial)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UModelFile new FModelMaterial cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
			Time1 = FDateTime::Now();
			if (newMtrl != NULL)
			{
				newMtrl->Serialize(true, Ar, m_Header.BodyVersion);
				UE_LOG(LogTemp, Log, TEXT("UModelFile Serialize FModelMaterial cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
				Time1 = FDateTime::Now();
			}
		...
		for (int32 iTexture = 0; iTexture < m_Header.Textures.Num(); ++iTexture)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UModelFile new FModelTexture cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
			Time1 = FDateTime::Now();
			newTexutre->Serialize(Ar, m_Header.BodyVersion);
			UE_LOG(LogTemp, Log, TEXT("UModelFile Serialize FModelTexture cost [%.2f] second"), (FDateTime::Now() - Time1).GetTotalSeconds());
			Time1 = FDateTime::Now();
...
USurfaceFile* UModelFile::DuplicateSurface(int32 MatSlotId, FString ChineseName, FString EnglishName)
{
	FSurfaceInfo ChineseNameInfo, EnglishNameInfo;
	ChineseNameInfo.Name = ChineseName;
	EnglishNameInfo.Name = EnglishName;

	return ExtractSurface(MatSlotId, ChineseNameInfo, EnglishNameInfo);
}

void UModelFile::UnloadResource()
{
	Unload();
}
...
FString UModelFile::GetUE4MaterialPath(int32 MaterialIndex)
{
	FModelMaterial *material = m_Materials.IsValidIndex(MaterialIndex) ? m_Materials[MaterialIndex] : NULL;
	if (material)
	{
		switch (material->GetType())
		{
			case EMaterialType::EMaterialUE4:
			{
				FModelMaterialUE4* M_UE4 = (FModelMaterialUE4*)material;
				if (M_UE4)
				{
					return M_UE4->UE4Path;
				}
				break;
			}
			case EMaterialType::EMaterialRef:
			{
				break;
			}
		 }
...
FString UModelFile::GetMaterialName(int32 MaterialIndex)
{
	FModelMaterial *material = m_Materials.IsValidIndex(MaterialIndex) ? m_Materials[MaterialIndex] : NULL;
		...
		if (material->Name.IsEmpty())
		{
			...
			if (material->GetType() == EMaterialRef)
			{
				...
				if (Surface)
				{
					if (Surface->DRInfo)
					{
						return Surface->DRInfo->GetStringField("ChineseName");
					}
					else
					{
						return Surface->GetSummary()->ResourceName;
					}
				}
			}
...
void UModelFile::SetMaterialName(int32 MaterialIndex, const FString &Name)
{
	FModelMaterial *material = m_Materials.IsValidIndex(MaterialIndex) ? m_Materials[MaterialIndex] : NULL;
	if (material)
	{
...
UTexture *UModelFile::GetMaterialPreviewImage(int32 MaterialIndex, int32 SizeX, int32 SizeY)
{
...
UTexture2D *UModelFile::GetTexureSlotInfo(int32 iMaterial, int32 iTexSlot, FString &TexSlotName, FString &TexName)
{
	UTexture2D *Tex = NULL;
	TArray<FTexSlotInfo> texSlots;
	...
	if (m_Materials.IsValidIndex(iMaterial))
	{
		...
		if (mtrl)
		{
			...
			FTexSlotInfo &SlotInfo = texSlots[iTexSlot];
			TexSlotName = SlotInfo.TexParamName.ToString();
			TexName = SlotInfo.TexValue->GetName();
			int32 TexValue = mtrl->GetTextureValue(SlotInfo.TexParamName);
			if (m_Textures.IsValidIndex(TexValue))
			{
				Tex = m_Textures[TexValue]->GetTexture();
			}
			else
			{
				if (mtrl->GetType() == EMaterialUE4)
				{
					Tex = Cast<UTexture2D>(SlotInfo.TexValue);
				}
				else if (mtrl->GetType() == EMaterialRef)
				{
					FModelMaterialRef *mtrlRef = (FModelMaterialRef*)mtrl;
					USurfaceFile *Surface = Cast<USurfaceFile>(mtrlRef->RefSurface.Get());
					if (Surface)
					{
						Tex = Cast<UTexture2D>(Surface->GetTexture(SlotInfo.iSlot));
					}
				}
			}
		}
...
FModelTexture *UModelFile::GetTextureByName(FModelMaterial *modelMaterial, const FName &TargetName)
{
	...
	if(!modelMaterial)	return modelTex;
...
UTexture2D *UModelFile::SetMaterialSlotTexture(int32 MaterialIndex, int32 TextureIndex, const FString &NewTexFile)
{
	...
	if (m_Materials.IsValidIndex(MaterialIndex))
	{
		...
		TArray<FTexSlotInfo> texSlots;
		material->GetRefTextures(texSlots, true);
...
UMaterialInstanceDynamic *UModelFile::GetUE4Material(int32 MaterialIndex)
{
	...
	if (material)
	{
		...
		DynamicMtrl = Cast<UMaterialInstanceDynamic>(ue4Mtrl);
		if (ue4Mtrl && !DynamicMtrl)
		{
...
{
	FVector NewOffset = GetOffsetByCenterType(InCenterType)*Scale3D;
...
void UModelFile::ResetSize()
{
	...
	DepthInMM = FMath::RoundToInt(Size.X)? FMath::RoundToInt(Size.X):1;
	WidthInMM = FMath::RoundToInt(Size.Y)? FMath::RoundToInt(Size.Y):1;
	HeightInMM = FMath::RoundToInt(Size.Z)? FMath::RoundToInt(Size.Z):1;
}
...
void UModelFile::CaclScale3D()
{
	...
	if (isnan(Scale3D.Z) || isinf(Scale3D.Z))
	{
		Scale3D.Z = 1;
	}
	MarkDirty();
...
FVector UModelFile::GetOffsetByCenterType(const FBox &Bounds, ECenterAdjustType InCenterType)
{
	FVector NewOffset;
...
{
	FBox	Bounds = GetLocalBounds();
	return GetOffsetByCenterType(Bounds, InCenterType);
}
