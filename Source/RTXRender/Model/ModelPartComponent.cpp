...
FModel *UModelPartComponent::GetModel()
{
	if (Model)
	{
		TArray<FModel *> &SubModels = Model->GetSubModels();
		if (SubModels.IsValidIndex(ModelIndex))
		{
			return SubModels[ModelIndex];
		}
	}
	return nullptr;
}

void UModelPartComponent::UpdateModel(const TArray<FPlane> &ClipPlanes, const FVector &Forward, float Len, UModelFile *InModel, int32 InModelIndex)
{
	Model = InModel;
