...
#define  MODELFILE_BODY_VER_8	8	// -> 9	 SurfaceObject  Name
#define  MODELFILE_BODY_VER		9	// 
...
public:	
	...
	//����UE4���ʵ�UE4Path
	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	FString GetUE4MaterialPath(int32 MaterialIndex);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	FString GetMaterialName(int32 MaterialIndex);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	void SetMaterialName(int32 MaterialIndex, const FString &Name);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	int32 GetNumerOfTexureSlot(int32 iMaterial);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	UTexture2D *GetTexureSlotInfo(int32 iMaterial, int32 iTexSlot, FString &TexSlotName, FString &TexName);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	UTexture2D *SetMaterialSlotTexture(int32 MaterialIndex, int32 TextureIndex, const FString &NewTexFile);
	...
	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	void SetPreviewImageData(int32 SizeX, int32 SizeY,const TArray<uint8> &ImageData);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	UTexture *GetMaterialPreviewImage(int32 MaterialIndex, int32 SizeX, int32 SizeY);
	...
	static FVector GetOffsetByCenterType(const FBox &Bounds, ECenterAdjustType InCenterType);

	UFUNCTION(BlueprintCallable, Category = "DR|Model")
	USurfaceFile* DuplicateSurface(int32 MatSlotId, FString ChineseName, FString EnglishName);
