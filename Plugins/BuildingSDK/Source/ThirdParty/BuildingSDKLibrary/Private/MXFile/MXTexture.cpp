...
void FTextureSourceInfo::Init(int Width, int Height, ETextureFormat InFormat, bool bSRGB, const char *InData, int NumBytes)
{
	SizeX = Width;
	SizeY = Height;
...
void FTextureSourceInfo::Serialize(ISerialize &Ar, int Ver)
{
	...
	else if (Ar.IsLoading())
	{
		...
		SRGB = LoadBool(Ar);
		CompressionNoAlpha = LoadBool(Ar);
		bDitherMipMapAlpha = LoadBool(Ar);
		bUseLegacyGamma = LoadBool(Ar);

		unsigned char ByteFormat = 0;
		Ar << ByteFormat;
		SourceFormat = (ByteFormat == 1) ? ETextureFormat::Tex_A8 : ETextureFormat::Tex_A8R8G8B8;
		...
		int NumImages = 0;
		Ar << NumImages;
		CompressedImages.resize(NumImages);
		...
		for (int i = 0; i < CompressedImages.size(); ++i)
		{
			...
			NumBytes = 0;
			Ar << NumBytes;
			Image.RawData.resize(NumBytes);
			Ar.Serialize(&Image.RawData[0], NumBytes);
		}
	}
}
...
void FModelTexture::Serialize(ISerialize &Ar, int Ver)
{
