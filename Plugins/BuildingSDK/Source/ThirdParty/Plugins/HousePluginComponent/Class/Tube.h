...
#define  TubeType  PLUGIN_SECTION_0 + 1

class  Tube :public IPluginObject
{
public:
	int GetType() { return TubeType; }
	...
	/*void SetTubeSurfArea(float area);
	float GetTubeArea();
...
class TubeDesc : public IPluginObjectDesc
{
public:
	bool IsResource() { return false; }
	const char *GetObjectClassName() { return "Tube"; }
	int	 GetObjectType() override { return TubeType; }
	IPluginObject *CreateObject()  override;
	void DestroyObject(IPluginObject *tObj) override;
};