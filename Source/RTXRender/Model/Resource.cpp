...
const TCHAR *GetResTypeName(EResType ResType)
{
	...
	/*case EResGZModel: return TEXT("mx.gz");*/
	default: break;
	}
	return TEXT("invalid");
}
...
void UResource::SetResourceFileName(const FString & InFileName)
{
	Filename = InFileName;
}
...
void UResource::NotifyPreviewImageChanged()
{
	if (PreviewImage)
	{
	...
	PreviewImage = GetPreviewTexture();

	if (PreviewImageChanged.IsBound())
	{
		PreviewImageChanged.Broadcast(this, PreviewImage);
	}
}
...
void UResource::ForceLoad()
{
	if (!bLoaded)
	{
		...
		if (Writer)
		{
			if (UResource::SkipMark(*Writer))
			{
				Serialize(*Writer);
				UE_LOG(LogTemp, Log, TEXT("ForceLoad, Serialize"));
				bLoaded = 1;
				bHeadLoaded = 1;
				OnPostLoad();
			}
