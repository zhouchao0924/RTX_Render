...
void FModelTexture::Decompress()
{
	if (Source.Data.Num() == 0 && Source.CompressedImages.Num() > 0)
	{
		...
		else if (PixelFormat == PF_BC5)
		{
			...
			IGeometry *GeoLib = SDK ? SDK->GetGeometryLibrary() : nullptr;
			if (GeoLib)
			{
				GeoLib->DecompressImageBC5(CompressedImage.SizeX, CompressedImage.SizeY, (unsigned char *)CompressedImage.RawData.GetData(), RGBAImage, false);
			}
		}
